...
internal struct HHLuanarCore { 
    private static let minYear        = 1890;     // 最小年限
    private static let maxYear        = 2100;     // 最大年限
    private static let weekStart      = 0;        // 周首日（可改成 app 配置）

    // Memory Cache
    private static let memoryCache = LCMemoryCache()
    // GMT 0 的时区
    private static let timeZone = TimeZone.init(secondsFromGMT: 0)
    // 错误码表
    private static let errorCode = [100:"输入的年份超过了可查询范围，仅支持1891至2100年",101:"参数输入错误，请查阅文档"]
    // 1890 ~ 2100 年农历新年数据
    private static let springFestival = [[1,21],[2,9],[1,30],[2,17],[2,6],[1,26],[2,14],[2,2],[1,22],[2,10],[1,31],[2,19],[2,8],[1,29],[2,16],[2,4],[1,25],[2,13],[2,2],[1,22],[2,10],[1,30],[2,18],[2,6],[1,26],[2,14],[2,4],[1,23],[2,11],[2,1],[2,20],[2,8],[1,28],[2,16],[2,5],[1,24],[2,13],[2,2],[1,23],[2,10],[1,30],[2,17],[2,6],[1,26],[2,14],[2,4],[1,24],[2,11],[1,31],[2,19],[2,8],[1,27],[2,15],[2,5],[1,25],[2,13],[2,2],[1,22],[2,10],[1,29],[2,17],[2,6],[1,27],[2,14],[2,3],[1,24],[2,12],[1,31],[2,18],[2,8],[1,28],[2,15],[2,5],[1,25],[2,13],[2,2],[1,21],[2,9],[1,30],[2,17],[2,6],[1,27],[2,15],  [2,3],[1,23],[2,11],[1,31],[2,18],[2,7],[1,28],[2,16],[2,5],[1,25],[2,13],[2,2],[2,20],[2,9],[1,29],[2,17],[2,6],[1,27],[2,15],[2,4],[1,23],[2,10],[1,31],[2,19],[2,7],[1,28],[2,16],[2,5],[1,24],[2,12],[2,1],[1,22],[2,9],[1,29],[2,18],[2,7],[1,26],[2,14],[2,3],[1,23],[2,10],[1,31],[2,19],[2,8],[1,28],[2,16],[2,5],[1,25],[2,12],[2,1],[1,22],[2,10],[1,29],[2,17],[2,6],[1,26],[2,13],[2,3],[1,23],[2,11],[1,31],[2,19],[2,8],[1,28],[2,15],[2,4],[1,24],[2,12],[2,1],[1,22],[2,10],[1,30],[2,17],[2,6],[1,26],[2,14],[2,2],[1,23],[2,11],[2,1],[2,19],[2,8],[1,28],[2,15],[2,4],[1,24],[2,12],[2,2],[1,21],[2,9],[1,29],[2,17],[2,5],[1,26],[2,14],[2,3],[1,23],[2,11],[1,31],[2,19],[2,7],[1,27],[2,15],[2,5],[1,24],[2,12],[2,2],[1,22],[2,9],[1,29],[2,17],[2,6],[1,26],[2,14],[2,3],[1,24],[2,10],[1,30],[2,18],[2,7],[1,27],[2,15],[2,5],[1,25],[2,12],[2,1],[1,21],[2,9]]
    // 农历数据
    private static let lunarCalendarData = [
    ...
    private static let worktime:[String:[String:Int]] = [
    ...
    // 星号表示不重要的节日
    // 破折号前面的是缩略写法
    private static let solarFestival = [
        "d0101":"元旦节",
        "d0120":"水瓶",
        "d0202":"湿地日-世界湿地日",
        "d0210":"*国际气象节",
    ...
    // 农历节日
    private static let lunarFestival = [
        "d0101":"春节",
        "d0115":"元宵节",
        "d0202":"龙抬头节",
        "d0505":"端午节",
    ...
    // 周节日
    private static let weekFestival = [
        "0513":"*世界哮喘日",
    ...
    * [闰月所在月，0为没有闰月; *正月初一对应公历月; *正月初一对应公历日; *农历每月的天数的数组（需转换为二进制,得到每月大小，0=小月(29日),1=大月(30日)）;]
    ...
    ///   - year:  公历年
    ///   - month:  公历月
    static func hh_calendar(_ _year: Int, _ _month: Int) -> Dictionary<String, Any>? {
        ...
        for i in 0..<monthData.count {
            ...
            let lunarData = hh_solarToLunar(cData["year"]!, cData["month"]!, cData["day"]!);
            array[i].merge(dict: lunarData)
        }
    ...
    static func hh_asyncCalendar(_ _year: Int,
                            _ _month: Int,
        ...
        var calendarData = solarCalendar(year, month + 1)
        //event/reminder/...
        var calendarDataList:[HHCalendarData] = []
        
        let queue = DispatchQueue(label: "com.hh.time.HHLuanarCore", qos: .default, attributes: .concurrent, autoreleaseFrequency: .workItem, target: nil)//并行队列
        ...
        queue.async(group: group, execute: DispatchWorkItem.init(block: {
            ...
            queue.async {
                ...
                HHCalendarManager.shared.loadCalendarData(year, month + 1) { (success, error, datas) in
                    guard let dataList = datas else {
                        group.leave()
                        semaphore.signal()
                        return
                    }
                    calendarDataList.append(contentsOf: dataList)
                    group.leave()
                    semaphore.signal()
                }
            }
            ...
            queue.async {
                ...
                for i in 0..<monthData.count {
                    let cData = monthData[i]
                    let lunarData = hh_asyncSolarToLunar(cData["year"]!, cData["month"]!, cData["day"]!, calendarDataList);
                    array[i].merge(dict: lunarData)
                }
                calendarData["monthData"] = array
                group.leave()
    ...
    ///   - month: 公历月
    ///   - day: 公历日
    static func hh_solarToLunar(_ _year: Int, _ _month: Int,_ _day: Int) -> [String : Any] {
        ...
        memoryCache.current = Int32(year)
        
        // 二十四节气
        var termList = [String:String]();
        let termListCache = memoryCache.get("termList")
            
        if (termListCache != nil) {
            termList = termListCache as! [String:String]
        } else {
        ...
        let lunarDate0 = lunarDate[0]
        let lunarDate1 = lunarDate[1]
        let lunarDate2 = lunarDate[2]
        ...
        if ((lunarDate1 == lunarMonthDays.count - 1) &&
            (lunarDate2 == lunarMonthDays[lunarMonthDays.count - 1])) {
            lunarFtv = lunarFestival["d0100"]
        }
        
        if (lunarLeapMonth > 0 && lunarDate1 >= lunarLeapMonth) {
        ...
        // 放假安排：0无特殊安排，1工作，2放假
        let yearKey = "y\(year)"
        let dayKey:String = formatDay(month, day)
        var workTime = 0;
        
        if let worktimeYear = worktime[yearKey],
            let hasData = worktimeYear[dayKey] {
            workTime = hasData
        }
    ...
    /// 公历转换成农历
       ...
       ///   - month: 公历月
       ///   - day: 公历日
       static func hh_asyncSolarToLunar(_ _year: Int,
                                        _ _month: Int,
                                        _ _day: Int,
                                        _ calendarDataList:[HHCalendarData]) -> [String : Any] {
           let inputDate = formatDate(_year, _month, _day);
        ...
        //自定义事件
         guard let customEvents = HHCalendarManager.shared.calendarDatasForDate(datas: calendarDataList, year, month + 1,day) else {
            return res
         }
         
        res = ["lunarDay": lunarDate[2],
                  "lunarMonthName": `$`(lunarMonthName),
                  "lunarDayName": `$`(lunarCalendarData["dateCn"]![lunarDate2 - 1]),
                  "solarFestival": i18n(`$`(solarFestival[formatDay(month,day)])),
                  "lunarFestival": `$`(i18n(lunarFtv ?? "")),
    ...
    ///   - month: 农历月
    ///   - day: 农历日
    static func hh_lunarToSolar(_ year: Int, _ month: Int,_ day: Int) -> Dictionary<String, Int> {
        let inputDate = formatDate(year, month, day);
...
//            return inputDate
        ...
        let timeInterval = TimeInterval(Int(buildDate(year, zenMonth - 1, zenDay).timeIntervalSince1970) + between * 86400);
        let offDate = Date.init(timeIntervalSince1970:timeInterval)
        let gregorian = Calendar.init(identifier: .gregorian)
        let comps = gregorian.dateComponents([.year,.month,.day], from: offDate)
    ...
    /// - Parameter key: key
    static func i18n(_ key: String)-> String {
        return key;
    }
    
    /// 获得不为空的字符串
    /// - Parameter text: text 
    static func `$`(_ text: String?)-> String {
        return text ?? "" ;
    }
    ...
    ///   - len: 参数
    ///   - start: 开始日期
    static func createMonthData(_ year:Int,_ month: Int,_ len:Int,_ start:Int) -> [[String:Int]] {
        ...
        let k = start;
        for i in 0..<len {
            let dict = ["year":year,"month":month,"day":(k+i)]
     ...
     *  return 格式化后的日期
     */
    static func formatDay(_ month:Int,_ day:Int)-> String {
        let monthString = String.init(format: "%02d", arguments: [month+1])
        let dayString = String.init(format: "%02d", arguments: [day])
        return "d"+monthString+dayString
    }
     ...
     *  @return NSDate
     */
    static func buildDate(_ year:Int, _ month:Int, _ day:Int)-> Date {
     ...
     *  @return NSDate
     */
    static func buildNewDate(_ year:Int, _ month:Int, _ day:Int)-> Date {
     ...
     *  @return 节日
     */
    static func getWeekFestival(_ year:Int, _ month:Int, _ day:Int)-> String? {
        let date = buildNewDate(year, month, day)
        let gregorian = Calendar.init(identifier: .gregorian)
        ...
        var weekDayCount = 0
        for curDay in 1...day {
            let curDate = buildNewDate(year, month, curDay)
            let weekComp = gregorian.dateComponents([.weekday], from: curDate)
            if weekComp.weekday == weekDay {
                weekDayCount = weekDayCount + 1
            }
        }
        let key = String.init(format: "%02d%d%d", arguments: [month,weekDayCount,weekDay])
        guard let festival = weekFestival[key] else { return nil }
     ...
     *  @return 闰月数 （月份从1开始）
     */
    static func getLunarLeapYear(_ year:Int)-> Int {
        let yearData = lunarInfo[year - minYear];
        return yearData[0];
    }
     ...
     *  @return 总天数
     */
    static func getLunarYearDays(_ year:Int)->[String:Any] {
        let yearData = lunarInfo[year - minYear]
        ...
        return ["yearDays":yearDays,"monthDays":monthDays]
    }
     ...
     *  @return 农历日期
     */
    static func getLunarDateByBetween(_ year:Int, _ between:Int)-> [Int] {
        ...
        for i in 0..<monthDays.count {
            let monthDaysI = Int(monthDays[i])
     ...
     *  @return 间隔天数
     */
    static func getDaysBetweenSolar(_ year:Int, _ month:Int, _ day:Int,
                                    _ year1:Int, _ month1:Int, _ day1:Int)-> Int {
        ...
        comp.month = month + 1
        comp.day = day
        let date = gregorian.date(from: comp)
        comp.year = year1
        comp.month = month1 + 1
        comp.day = day1;
        let date1 = gregorian.date(from: comp)
     ...
     *  @return 农历日期
     */
    static func getLunarByBetween(_ year:Int, _ month:Int, _ day:Int)-> [Int] {
        let yearData = lunarInfo[year - minYear];
        let zenMonth = yearData[1]
        let zenDay = yearData[2]
     ...
     *  @return 日期: 节气名
     */
    static func getYearTerm(_ year:Int)-> [String : String] { 
        var res = [String:String]()
        guard let keys = solarTerms[year] else {
            return res
        }
        guard let values = lunarCalendarData["solarTerm"] else {
            return res
        }
     ...
     *  @return 生肖
     */
    static func getYearZodiac(_ year:Int)-> String {
     ...
     *  @return 天干地支
     */
    static func cyclical(_ num:Int)-> String {
        let tiangan = i18n(lunarCalendarData["heavenlyStems"]![num % 10]);
        let dizhi = i18n(lunarCalendarData["earthlyBranches"]![num % 12]);
     ...
     *  @return 干支纪年
     */
    static func getLunarYearName(_ year:Int, _ offset:Int)-> String{
     ...
     *  @return 该月天数
     */
    static func getSolarMonthDays(_ year:Int, _ month:Int)-> Int {
        if (month == 1) { // 二月 闰月处理
     ...
     *  @return 格式化后的日期
     */
    static func formatDate(_ year:Int, _ month:Int, _ day:Int)->[String:Any] {
        let now = Date()
        var gregorian = Calendar.init(identifier: .gregorian)
        gregorian.timeZone = timeZone!
               
        let comps = gregorian.dateComponents([.year,.month,.day], from: now)
        let _year = year
        let _month = month - 1
        let _day = (day > 0) ? day: comps.day!
         
        if (year < (minYear > 0 ? minYear : minYear + 1) || year > maxYear) {
            return ["error": 100, "msg": errorCode[100] ?? "未知错误"];
        }
     ...
     *  @return YES 表示处于农历新年
     */
    static func isNewLunarYear(_ _year:Int, _ _month:Int, _ _day:Int)-> Bool {
        let springFestivalDate = springFestival[_year - minYear];
     ...
     *  @return 公历
     */
    static func solarCalendar(_ _year:Int, _ _month:Int)-> [String:Any] {
        let inputDate = formatDate(_year, _month, -1);
        if inputDate["error"] != nil {
            return inputDate
        }
        ...
        let year = inputDate["year"] as! Int;
        let month = inputDate["month"] as! Int;
        let firstDate = buildDate(year, month, 1)
        
        var res = ["firstDay": (getDay(firstDate)), // 该月1号星期几
                    "monthDays": (getSolarMonthDays(year, month)), // 该月天数
            "monthData": [[String:Int]]()] as [String : Any]
        ...
        if identifier.hasSuffix("japanese") || identifier.hasSuffix("buddhist") {
            firstDay = firstDay + 1 // 处理日本日历和佛教日历
        }
        ...
        var preMonthData = createMonthData(preYear, preMonth + 1, preFillDays, preMonthDays - preFillDays + 1)
         
        preMonthData.append(contentsOf:res["monthData"] as! [[String:Int]])
        res["monthData"] = preMonthData
        ...
        if (fillLen != 0) {
            ...
            var monthData = res["monthData"] as! [[String:Int]]
            monthData.append(contentsOf:nextMonthData);
            res["monthData"] = monthData
        }
    ...
    /// 计算农历日期离正月初一有多少天
    /// @param year 农年
    /// @param month 月(0-12，有闰月)
    /// @param day 日
    static func getDaysBetweenZheng(_ year:Int, _ month:Int, _ day:Int)-> Int {
        let lunarYearDays = getLunarYearDays(year);
        let monthDays = lunarYearDays["monthDays"] as! [Int];
        var days = 0
